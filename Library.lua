...
--[[
    You can access Toggles & Options through (I'm planning to remove **a** option):
        ...
        b) getgenv().Worm.Toggles, getgenv().Worm.Options
...
getgenv().Toggles = Toggles; -- if you load infinite yeild after you executed any script with WormLib it will just break the whole UI lib :/ (thats why I added getgenv().Worm)
        ...
        if Toggled then
            ...
            if DrawingLib.drawing_replaced ~= true and IsBadDrawingLib ~= true then
                IsBadDrawingLib = not (pcall(function()
                    ...
                    pcall(function() RunService:UnbindFromRenderStep("WormCursor") end)
                    RunService:BindToRenderStep("WormCursor", Enum.RenderPriority.Camera.Value - 1, function()
                        ...
                        if not Toggled or (not ScreenGui or not ScreenGui.Parent) then
                            ...
                            RunService:UnbindFromRenderStep("WormCursor")
                        end
                    end)
                end));
            end
...
if getgenv().skip_getgenv_worm ~= true then getgenv().Library = Library end
return Library
